...
class MMCYixin189Email:
    ...
    def action(self, d, z,args):
        ...
        if d(text="邮箱提醒",resourceId="im.yixin:id/module_title").exists:
            ...
            z.sleep(1)
        number = ""
        if d(textContains="@189.cn",resourceId="im.yixin:id/plug_mail_list_account").exists:
            d(textContains="@189.cn",resourceId="im.yixin:id/plug_mail_list_account").click()
        ...
        for i in range(0,count):
            ...
            if d(text="写邮件",resourceId="im.yixin:id/mail_write").exists:
                d( text="写邮件", resourceId="im.yixin:id/mail_write" ).click()
            else:
                z.toast("没找到写邮件按钮")
                return
            ...
            if len( Material2 ) == 0:
                d.server.adb.cmd( "shell","am broadcast -a com.zunyun.zime.toast --es msg \"%s号仓库为空，没有取到消息\"" % repo_material_cateId2 ).communicate( )
                z.sleep( 10 )
                return

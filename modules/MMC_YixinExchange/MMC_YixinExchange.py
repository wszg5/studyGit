...
class MMC_YixinExchange:
    ...
    def action(self, d, z,args):
        ...
        d.press.back( )
        if d( description="电话短信" ).exists:
            d( description="电话短信" ).click( )
            z.sleep( 0.5 )
        if d( description="短信包" ).exists:
            d( description="短信包" ).click( )

        moneyExchange = args["moneyExchange"]
        if d( description=moneyExchange ).exists:
            d( description=moneyExchange ).click( )
            z.sleep( 4 )
            if d( description="马上抢" ).exists:
                d( description="马上抢" ).click( )
                z.sleep( 1 )
                d.click( 369 / 720 * width, 1197 / 1280 * height )
                z.sleep( 3 )
                if d( description="立即支付" ).exists:
                    d( description="立即支付" ).click( )

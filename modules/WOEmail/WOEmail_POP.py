...
class WOEmail_POP:
    def __init__(self):
        ...
        self.popHost = 'pop.wo.cn'
        self.smtpHost = 'smtp.wo.cn'
        self.port = 25
    ...
    for num in range(1,index + 1):
        ...
        if "From:" in lines[25]:
            try:
                ...
                # print lines[25]
                print lines[25].split("From: ")[1]
        elif "From:" in lines[30]:
            try:
                b = lines[30].split( "<" )[1]
                print b.split( ">" )[0]
            except:
                # print lines[30]
                print lines[30].split( "From: " )[1]
            # lines存储了邮件的原始文本的每一行,
        # 可以获得整个邮件的原始文本:
        # msg_content = '\r\n'.join( lines )
        # # 稍后解析出邮件:

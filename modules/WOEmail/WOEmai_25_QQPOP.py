        ...
        try:
            ...
            while True:
                ...
                for i in range(0, sendCount + 1):
                    ...
                    if flag:
                        while True:
                            numbers = self.repo.GetAccount("normal", "531", 0, 1)
                            if len(numbers) == 0:
                                numbers = self.repo.GetAccount("normal", "531", 0, 1)
                                if len(numbers) == 0:
                                    print u"%s号仓库没有数据,等待5分钟" % "531"
                                    time.sleep(300)
                                    return

                            toAccount = numbers[0]['number']  # 即将登陆的QQ号
                            toPassword = numbers[0]['password']
                            if "@" not in toAccount:
                                toAccount = toAccount + "@qq.com"

                            pop3_server = "pop.qq.com"
                            # toAccount = "1077469875@qq.com"
                            # toPassword = 'wdtmvkqscgvjihhd'
                            sendMailArr = []
                            ...
                            try:
                                ...
                                # 身份认证:
                                server.user(toAccount)
                                server.pass_(toPassword)
                                # stat()返回邮件数量和占用空间:
                                ...
                                # 获取最新一封邮件, 注意索引号从1开始:
                                index = len(mails)
                                print u"%s pop收信登录成功" % toAccount
                                break
                            except Exception, e:
                                print u"%s pop收信登录失败" % toAccount
                                server.quit()
                                try:
                                    # 连接到POP3服务器:
                                    server = poplib.POP3_SSL(pop3_server, 995)
                        ...
                        try:
                            e = self.send(message, my_sender, password, [toAccount], theme, [], name, imgpath,fjFlag)
                            time.sleep(random.randint(sleepStart,sleepEnd))
                        except Exception, e:
                            flag = False
                    else:
                        try:
                    ...
                    if e != None:
                        ...
                        if flag:
                            ...
                            if my_sender not in sendMailArr:
                                print u"%s 发给%s 没进收件箱"%(my_sender,toAccount)
                                flag = True
                                asdlFalg = False
                                return
                            else:
                                print u"%s 发给%s 进了收件箱" % (my_sender, toAccount)
                            for num in range(1, index + 1):
                                server.dele(num)
                            print u"删信。。。"
                            server.quit()

...
class TIMAppointGroupChatPullGroupFriends:
    ...
    def pullFriends(self, d,z, args,myAccount,totalNumber):
        ...
        qq = self.repo.GetNumber( repo_qq_id, 0, 2000, "normal", "NO", None, groupNumber )  # 取出t1条两小时内没有用过的号码
        if len( qq ) == 0:
    ...
    def getGroupNumber(self,d,z,args,myAccount):
        ...
        group = self.repo.GetNumber( repo_group_id, 0, 1, "normal", "NO",
...
if __name__ == "__main__":
    ...
    #
    # para = {"x_key": "", "x_value": ""}
    # totalList = Repo( ).GetTIMInfomation( "260", para )
    #
    # if len( totalList ) == 0:
    #     z.toast( "%s仓库%s账号可用数据为空" % ("260", "44556644") )
    # num = random.randint( 0, len( totalList ) - 1 )
    # address = totalList[num]["phonenumber"]
    # address = address.encode( 'utf-8' )
    # if totalList[num]["x03"] == None:
    ...
    # if useCount >= 5:
    #     if thisTime == None:
    #         para = {"phoneNumber": address,
    #                 'x_03': useCount, 'x_04': nowTime}
    #         z.toast( "该地址已被超过五人使用,3小时候后才可被使用" )
    #         Repo().PostInformation( "260", para )
    #     thisTime = o.getTimeSecond( thisTime )
    #     nowTime = o.getTimeSecond( nowTime )

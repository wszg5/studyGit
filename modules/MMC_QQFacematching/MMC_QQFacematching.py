...
class MMCQQFacematching:
    ...
    def action(self, d, z,args):
        ...
        # else:
        #     pass
        if d(textContains="颜值配对",className="android.widget.TextView").exists:
            d( textContains="颜值配对", className="android.widget.TextView" ).click()
            z.sleep(12)
        else:
            z.toast("没有颜值匹配,停止模块")
            now = datetime.datetime.now( )
        ...
        while t < textnum:
            ...
            if colorResource3 == (18, 183, 245):
                ...
                z.toast( '模块结束，保存的时间是%s' % nowtime )
                return
            colorResource = self.WebViewBlankPages( d, 338 / 1280 * height, 178 / 720 * width, 506 / 720 * width,570 / 1280 * height )

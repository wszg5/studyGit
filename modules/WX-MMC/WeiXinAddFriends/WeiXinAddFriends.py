...
class WeiXinAddFriends:
    ...
    def action(self, d,z, args):
        ...
        if d(index='1', className='android.widget.TextView').exists:
        ...
        while True:
            ...
            if account < add_count:
                ...
                if d(textContains='用户不存在').exists:
                    d(resourceId='com.tencent.mm:id/baw').click()
                    z.sleep(1)
                    continue
                ...
                if d(text='添加到通讯录').exists:      #存在联系人的情况
                    ...
                    if d(text='发消息').exists:
                        ...
                        z.sleep(1)
                        continue
                    # obj = d(className='android.widget.EditText', resourceId='com.tencent.mm:id/cph').info  # 将之前消息框的内容删除
                    # obj = obj['text']
                    # lenth = len(obj)
                    # t = 0
                    # while t < lenth:
                    #     d.press.delete()
                    #     t = t + 1
                    d(className='android.widget.EditText', resourceId='com.tencent.mm:id/d0j').click()
                    z.sleep(1)
                    d(className='android.widget.EditText', resourceId='com.tencent.mm:id/d0j').click.bottomright()
                    z.input(message)
                    if args["set_remark"] == "是":
                        d(className='android.widget.EditText', resourceId='com.tencent.mm:id/d0n').click()
                        z.sleep(1)
                        d(className='android.widget.EditText', resourceId='com.tencent.mm:id/d0n').click.bottomright()
                        z.input(WXnumber)
                    d(text='发送').click()
                    z.sleep(3)
                    z.heartbeat()
...
if __name__ == "__main__":
    ...
    args = {"repo_number_cate_id": "104", "repo_material_cate_id": "40", 'run_time_min': '0', 'run_time_max': '0', 'start_time': '', 'stop_time': '', "add_count": "3","set_remark": "是", 'gender': "女",  "time_delay": "3"}    #cate_id是仓库号，length是数量

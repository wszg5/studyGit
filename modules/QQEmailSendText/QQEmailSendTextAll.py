...
class QQEmailSendTextAll:
    ...
    def send_mail(self, d, z, args, fj, repo_mail_cateId, repo_mail_cateId2, account, check=True):
        ...
        if check:
            if (args["time_delay"]):
                z.toast("等待%s秒后查看收件箱"%int(args["time_delay"]))
                time.sleep(int(args["time_delay"]))
            d.server.adb.cmd( "shell",
                              "am start -n com.tencent.androidqqmail/com.tencent.qqmail.LaunchComposeMail" ).communicate( )  # 拉起QQ邮箱
            time.sleep( 2 )
            accountObj = d(resourceId="com.tencent.androidqqmail:id/ac",textContains="@",className="android.widget.TextView")
            # accountArr = accountObj.info["text"]
            # if "@qq.com" in accountArr or "@163.com":
            #     account = accountArr.split("@")[0]
            # else:
            #     account = accountArr
            # z.toast("查看是否进收件箱")
    ...
    def action(self, d, z, args):
        ...
        auto_reply_cate_id = args['auto_reply_cate_id']
        ...
        success4 = False
        success5 = False
        z.toast("给正常号码和自动回复号码『不带附件』发消息")
        for sc in range(4):
            z.heartbeat( )
            result = self.send_mail(d,z,args,fj,repo_mail_cateId,auto_reply_cate_id,account)
            if result=='over':
                return
            elif result=='success':
                d.press.back( )
                ...
                if sc==0:
                    fj = True
                    z.toast( "给正常号码和自动回复号码『带附件』发消息" )
                elif sc==1:
                    fj = False
                    repo_mail_cateId = ''
                    z.toast( "给自动回复号码『不带附件』发消息" )
                elif sc==2:
                    fj = True
                    z.toast( "给自动回复号码『带附件』发消息" )
                else:
                    fj = True
                    success4 = True
                    z.toast( "给小号『带附件』发消息" )
        ...
        for sc in range( sendCount ):
            z.heartbeat( )
            result = self.send_mail( d, z, args, fj, repo_mail_cateId, auto_reply_cate_id, account, False )
            if result=='over':
                return
...
def getPluginClass():
    return QQEmailSendTextAll

if __name__ == "__main__":
    clazz = getPluginClass()
    o = clazz()
    d = Device("HT52TSK00491")
    z = ZDevice("HT52TSK00491")
    ...
    args = {"repo_mail_cateId": "399", "repo_material_cateId": "382", "time_delay": "5","sendCount":"6","size":"实际大小","repo_material_cateId2":"383","selectContent":"主题内容都发",
            "failCount":"7","nuberLoop":"循环","addToText":"添加",'auto_reply_cate_id':"408",'train_cate_id':"411",'random_code':"不乱码"}
    # Str = d.info  # 获取屏幕大小等信息
    # height = int( Str["displayHeight"] )
    # width = int( Str["displayWidth"] )

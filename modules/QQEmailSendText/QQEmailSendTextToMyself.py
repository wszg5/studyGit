...
class QQEmailSendTextToMyself:
    ...
    def action(self, d, z, args):
        ...
        account = accountObj.info["text"].encode("utf-8")
        account = account.split( "@" )[0]
        # account = accountArr[0]
        sendCount_x, sendCount_y= args["sendCount"].split("-")


        sendCount = random.randint(int(sendCount_x),int(sendCount_y))
        # accountObj = d( resourceId="com.tencent.androidqqmail:id/ac", textContains="@",
        #                 className="android.widget.TextView" )
        ...
        for sc in range(sendCount):
            ...
            if flag2 or flag1:
                ...
                self.input(random_code,z, account )
...
def getPluginClass():
    return QQEmailSendTextToMyself

if __name__ == "__main__":
    clazz = getPluginClass()
    o = clazz()
    d = Device("HT537SK00603")
    z = ZDevice("HT537SK00603")
    d.server.adb.cmd("shell", "ime set com.zunyun.qk/.ZImeService").communicate()
    args = {"repo_material_cateId": "382", "time_delay": "3","sendCount":"1-3","picture":"是","size":"实际大小","repo_material_cateId2":"383","selectContent":"主题内容都发","random_code":"不乱码"}
    # Str = d.info  # 获取屏幕大小等信息
    # height = int( Str["displayHeight"] )
    # width = int( Str["displayWidth"] )

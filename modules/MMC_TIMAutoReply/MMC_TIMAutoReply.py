    ...
    def action(self, d, z,args):
        ...
        while i < 200:
            ...
            if 'icmp_seq' and 'bytes from' and 'time' in ping[0]:
                z.toast( "网络通畅。开始执行：准备执行MMS版TIM监控自动回复" )
                break
            z.sleep( 2 )
        if i > 200:
        ...
        while i < add_count:
            if k == 10:
                ...
                z.toast( '模块结束，保存的时间是%s' % nowtime )
                return
            if d(resourceId="com.tencent.tim:id/no_chat",text="暂时没有新消息").exists:
                # d( resourceId="com.tencent.tim:id/no_chat", text="暂时没有新消息" ).click()
                z.toast( "无消息,等待10秒" )
                z.sleep( 10 )
                z.heartbeat()
                k = k + 1
                continue
            k = 0
            obj = d( resourceId="com.tencent.tim:id/recent_chat_list", index=1,
                     className="android.widget.AbsListView" ).child( index=2, className="android.widget.LinearLayout" )
            if obj.exists:
                z.heartbeat( )
                ...
                obj2 = obj.child( index=0, resourceId="com.tencent.tim:id/relativeItem",
                                  className="android.widget.RelativeLayout" ).child( index=1,
                                                                                     className="android.widget.RelativeLayout" ).child(
                    index=2, resourceId="com.tencent.tim:id/unreadmsg", className="android.widget.TextView" )
                z.heartbeat( )
                if obj2.exists:
                    obj.child( index=0, resourceId="com.tencent.tim:id/relativeItem",
                               className="android.widget.RelativeLayout" ).child( index=1,
                                                                                  className="android.widget.RelativeLayout" ).click( )
                    if d( resourceId='com.tencent.tim:id/input', className="android.widget.EditText" ).exists and not d(resourceId="com.tencent.tim:id/ivTitleBtnRightImage",description="群资料卡").exists and not d(text="TIM团队",resourceId="com.tencent.tim:id/title").exists:
                    ...
                    z.sleep( 1 )
                    if d( resourceId="com.tencent.tim:id/name", description="删除" ).exists:
                        d( resourceId="com.tencent.tim:id/name", description="删除" ).click( )
                        z.sleep( 1 )
                    else:

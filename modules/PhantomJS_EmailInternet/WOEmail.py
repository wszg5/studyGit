...
class WOEmail:
    ...
    def send(self,message,my_sender,password,my_user,theme,bcc,name):
        ret = True
        try:
            ...
            file_name = "/home/zunyun/text/zunyun.png"
    ...
    def sendProcess(self, args):
        try:
            ...
            while True:
                ...
                numbers = self.repo.GetAccount( repo_account_cate_id, 30, 1 )
                if len( numbers ) == 0:
                ...
                e = self.send(message,my_sender,password,my_user,theme,bcc,name)
                # signal.alarm( 0 )
                if e !=None:
                    print e
                    ex = ""
    ...
    def action(self):
        ...
        while True:
            ...
            if param["x01"] is None or param["x01"]=="":
                print u"不能没有收件人,请到公网上修改邮件任务参数信息"
                time.sleep(300)
                continue
            else:
                my_userCount = int(param["x01"])
            if param["x02"] is None or param["x02"]=="":
                bccCount = 0
            else:
                bccCount = int(param["x02"])
            emailType = param["x03"]

            args = {"repo_account_cate_id": repo_account_cate_id,
                    "repo_number_cate_id": repo_number_cate_id, "repo_theme_cateId": repo_theme_cateId,
                    "repo_message_cateId": repo_message_cateId,"repo_bcc_cate_id":repo_bcc_cate_id,"my_userCount":my_userCount,"bccCount":bccCount,"emailType":"QQ邮箱","repo_name_cateId":repo_name_cateId}  # cate_id是仓库号，length是数量
